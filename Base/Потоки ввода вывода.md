...
#do/review взять из ukae из cpp картинки на тему потоком, мб текст еще, посмотреть в общем

# Что такое потоки?
Потоки можно сказать - это каналы, которые либо посылают данные на выход(out), либо принимают данные на вход(in)
...
Разница между буферизированными и небуферизированными в том, что буферизированные не сразу выводят данные, а сохраняют их в своем буфере, как только буфер переполняется, либо происходит сброс буфера, данные выводятся. Если для перевода строки использовать \n сброс буфера происходить не будет.
...
# stringstreams
Поскольку istringstream и ostringstream наследуеются от istream и ostream $=>$ имеют схожее поведение.
...
1. seekx() - позволяет переместиться в произвольную позицию внутри входного или выходного потока, для входного потока seekg(), для выходного seekp(), почему их два? Потому что файловый поток и входной и выходной, он хранит данные и теущей позиции для чтения и записи
2. seekg() и seekp() имеют по 2 перегрузки которые принимают
	1. Один аргумент - принимает позиция(тип std::streampos) и выполняет поиск по ней
	2. Два аргумента - перемещает на относительную позицию в потоке, принимает смещение(тип std::streamoff) и позицию(тип std::streampos) и выполняет поиск по ней
...
## Библиотека поддержки файловой системы
### Путь
...
#do/review распределить по файлам