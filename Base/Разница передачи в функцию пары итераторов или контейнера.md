...
```cpp
static void TransIter(benchmark::State& state) {  
    Matrix<int> A, B, C;  
    A.readMatrix();  
    B.readMatrix();  
    C.resizeto(A.x(), A.y());  
...
static void TransMatrix(benchmark::State& state) {  
    Matrix<int> A, B, C;  
    A.readMatrix();  
    B.readMatrix();  
    C.resizeto(A.x(), A.y());  
...
BENCHMARK(TransMatrix);
```
![[../Files/Pasted image 20220425140609.png]]
[Quick bench](https://quick-bench.com/q/CHQ8SDUL9RvBxaatznJ438Dij00)
Разница более чем в три раза при передачи итераторов вместо контейнера. Все из-за уровня косвенности, передавая контейнер мы лезем в него за итератором и разыменовываем его, передавая итератор мы сразу разыменовываем его.