...
`copy_backward()` - копирует элементы начиная с последнего

#do/review существуют какие-то огранчения, посмотреть на cppreference
...
`unique()` - удаляет все дублирующиеся смежные элементы, поэтому нужно использовать его на отсортированной последовательности. Первый дубликаты он удаляет, последующие перемещает в конец
...
`unique_copy()` - копирует результат в другой контейнер
...
`shuffle()` переставляет элементы диапазона в случайном порядке с линейной сложностью
...
`sample()` - возвращает выборку из n случайно выбранных элементов из заданного диапазона источника и сохраняет их в диапазон назначения.
...
`partition_copy()` - копирует элементы из источника в два разных места назначения. Место назначения выбирается на основе результата предиката(true, false). Возвращает пару итераторов - последний скопированный элемент в одном и втором месте.
...
`partition()` - сортирует последовательность - сначала идут элементы для которых предикат true, потом для которых предикат false, порядок элементов не сохраняется.
...
# Sorting Algorithms
`sort()` сортирует последовательность обычно за O(NlogN)
...
*/
```
`stable_sort()` - менее эффективен чем `sort()` поскольку сохраняет относительный порядок одинаковых элементов в диапазоне


`nth_element()` - алгоритм выбора, сортирует последовательность так, что перед nth элементом все что меньше его, а после все что больше его
...
#do/review Я блять не понимаю, почему он нормально не сортирует, почему в последней отсортированной последовательноти в конце 2,3,4. Эта хуйня сортирует относительно 3, почему 2 больше 3 а 4 нет???
...
`min_element() max_elemnt()` - возвращают итератор на наименьший, наибольший элемент
...
`clamp()` - принимает 3 аргумента - val, lo, hi - возвращает ссылку на lo, если $v<lo$, возвращает ссылку на hi, если $v>hi$, иначе возвращает ссылку на v.
```cpp
...
| Тип правила                 | Глобальный экземпляр | Описание                                                      |
| --------------------------- | -------------------- | ------------------------------------------------------------- |
...
`iota` - генерирует последовательность иp заданного значения постоянно прибавляя значение на 1
...
`reduce()`
В отличии от accumulate, reduce поддерживает параллельное выполнения
...
`transofrm_reduce()` - поддерживает параллельное выполнение, может выполнять как на одном интервале, так и на двух(поведение такое же как и у `inner_product()` за исключением того, что по умолчанию он использует бинарные операторы `std::plus и std::multiplies`, вместо `operator+ и operator*` у `inner_product()`)
...
1. exclusive_scan() - $y_0=Init, y_1=InitѲx0, y_2=InitѲx0Ѳx1$
2. inclusive_scan()/partial_sum() - $y_0=InitѲx0, y1=InitѲx0Ѳx1$
