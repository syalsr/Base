# Пишем свой итератор
Если мы пишем свой итератор в нем должны быть определены:
1. `iterator_category` – категория нашего итератора 
...
int main() {  
    ...
    for (auto &&both : itertools::make_zip_range(k, v))  
        std::cout << both.first << " " << both.second << "\n";  
    //42 4.5  
    //2 3.5    //3 2.5    //4 1.5        // 3. wrong iterators don't work  
#ifdef BAD  
    auto osit = std::ostream_iterator<int>{std::cout};  
    auto wit = itertools::make_zip_iterator(osit, b.begin());#endif  
}
```

Мы сделали итератор адаптер - он строит итератор на основе двух других итераторов не прибегая к созданию контейнера