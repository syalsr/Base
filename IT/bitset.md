# bitset
bitset это альтернатива array то есть у него фиксированный размер, являющийся параметром контейнера. При этом он хранит данные более компактно (как vector). По умолчанию значения контейнера 0, можно передать строку состоящую из 0 и 1, что бы конструктор преобразовал ее в обратном порядке, т.е. если `str="001"`, то `bitset=100`.
